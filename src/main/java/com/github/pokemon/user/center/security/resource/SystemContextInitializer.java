...
public class SystemContextInitializer implements CommandLineRunner {
    ...
    private SystemRoleDO getRole() {
        return SystemRoleDO.builder().roleName("超管").authority("ROLE_ROOT").build();
    }

    private SystemUserDO getUser() {
        return SystemUserDO.builder().username("root").phone("18867110336").password(("123456")).email("spring-boot@qq.com").build();
    }
